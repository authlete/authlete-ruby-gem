...
class AuthorizationResponseTest < Minitest::Test
  ...
  def test_from_json
    jsn = generate_json()
    hsh = JSON.parse(jsn)
    obj = Authlete::Model::Response::AuthorizationResponse.new(hsh)
  ...
  def test_setters
    obj = Authlete::Model::Response::AuthorizationResponse.new
    obj.acrs                 = ACRS
    obj.action               = ACTION
    obj.claimsLocales        = CLAIMS_LOCALES
    obj.claims               = CLAIMS
    obj.display              = DISPLAY
    obj.idTokenClaims        = ID_TOKEN_CLAIMS
    obj.maxAge               = MAX_AGE
    obj.prompts              = PROMPTS
    obj.purpose              = PURPOSE
    obj.requestObjectPayload = REQUEST_OBJECT_PAYLOAD
    obj.resources            = RESOURCES
    obj.responseContent      = RESPONSE_CONTENT
    obj.service              = SERVICE
    obj.scopes               = SCOPES
    obj.subject              = SUBJECT
    obj.ticket               = TICKET
    obj.uiLocales            = UI_LOCALES
    obj.userInfoClaims       = USER_INFO_CLAIMS
